...
def test_time_it_incorrect_pos_args():
...
def test_squared_power_list_no_args():
...
def test_squared_power_list_incorrect_pos_args():
...
def test_squared_power_list_incorrect_start__end():
...
def test_squared_power_list_start_gt_end():
...
def test_squared_power_list_number_gt_10():
...
def test_squared_power_list_unwanted_args():
...
def test_squared_power_list_output():
...
def test_polygon_area_length():
    ...
    with pytest.raises(TypeError, match=r".*Length can be a real number, int or float type only*"):
...
def test_polygon_area_sides():
    ...
    with pytest.raises(TypeError, match=r".*Side can be of integer type only*"):
        session5.polygon_area(10,sides='ten')
    with pytest.raises(TypeError, match=r".*Side can be of integer type only*"):
        session5.polygon_area(10,sides=1+2j)

def test_polygon_area_sides_values():
    """Test polygon_area function for permissible values for sides"""
    with pytest.raises(ValueError, match=r".*Number of sides must be between [3-6]*"):
...
def test_polygon_area_length_values():
    """Test polygon_area function for permissible values for sides"""
    with pytest.raises(ValueError, match=r".*Length should be greater than 0*"):
        session5.polygon_area(-1,sides=4)

def test_polygon_area_unwanted_args():
...
def test_polygon_area_output():
    ...
    assert session5.polygon_area(2,sides=3) == 1.7320508075688772, "polygon_area is not working as expected"
    assert session5.polygon_area(2,sides=4) == 4, "polygon_area is not working as expected"
    assert session5.polygon_area(2,sides=5) == 6.881909602355868, "polygon_area is not working as expected"
    assert session5.polygon_area(2,sides=6) == 10.392304845413264, "polygon_area is not working as expected"
...
def test_temp_converter_temp():
    """Test temp_converter function for incorrect values for positional argument temp"""
    with pytest.raises(TypeError, match=r".*Temprature can be a real number, int or float type only*"):
        session5.temp_converter('sac')
    with pytest.raises(TypeError, match=r".*Temprature can be a real number, int or float type only*"):
        session5.temp_converter(1+2j)

def test_temp_converter_temp_given_in():
...
def test_temp_converter_temp_values_in_celsius():
...
def test_temp_converter_temp_values_in_fahrenheit():
...
def test_temp_converter_unwanted_args():
...
def test_temp_converter_output_in_fahrenheit():
...
def test_temp_converter_output_in_celsius():
...
def test_speed_converter_speed_allowed_values():
...
def test_speed_converter_time_allowed_values():
...
def test_speed_converter_dist_allowed_values():
...
def test_speed_converter_unwanted_args():
...
def test_speed_converter_output_km_to():
...
def test_speed_converter_output_m_to():
...
def test_speed_converter_output_ft_to():
...
def test_speed_converter_output_yrd_to():
